...
class GPTOSSToolCaller:
    def __init__(self, model_id: str = "gpt-oss-120b"):
        """Initialize GPT-OSS model with Cerebras Cloud SDK."""
        self.model_id = model_id
        self.client = Cerebras()
        
        # Initialize GitHub token store
...
if __name__ == "__main__":
    ...
    messages = [
        {
            "role": "user", 
            "content": "Create a pull request for the repository 'microsoft/vscode' with the title 'Add new feature' and body 'This PR adds a new feature to improve user experience.'"
        }
    ]
    
    # Call tools with streaming
    print("Streaming response:")
    for chunk in caller.call_tools_stream(messages, reasoning_level="high"):
        print(chunk, end="", flush=True)
    
    print("\n\nNon-streaming response:")
    result = caller.call_tools(messages, reasoning_level="high", stream=False)
